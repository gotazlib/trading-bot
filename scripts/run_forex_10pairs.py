"""Brute-Force-Pair-Analyse auf 10 Forex-Pairs (2010-2026).

Erweiterung von run_forex_brute_force.py: 5 Majors + 5 Crosses.
Mehr Diversifikation, mehr Daten fuer robuste Indikator-Paare.
...
- 2x Hebel (konservativ fuer Forex)
"""
import itertools
import warnings
...
TP_PCT = 0.015             # 1.5 % TP (Forex bewegt sich kleiner)
SL_PCT = 0.0075            # 0.75 % SL (2:1 Reward)
MAX_HOLD_BARS = 30
LEVERAGE = 2.0
...
TOP_N_PAIRS = 10
MIN_VOTES_PCT = 0.30
MIN_TRADES_PER_PAIR = 10   # mehr Daten → strenger
...
def main():
    ...
    print(f"Davon {len(robust)} Paare auf ALLEN {len(pairs_data)} Forex-Pairs profitabel.")
    ...
    print(f"\n=== TOP-{min(TOP_N_PAIRS, len(robust))} ROBUSTE PAARE (alle {len(pairs_data)} Forex-Pairs positiv) ===")
