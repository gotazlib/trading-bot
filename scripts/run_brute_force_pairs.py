"""Brute-Force Multi-Coin Indikator-Paar-Analyse.

Vorgehen (statistisch sauber):
1. Alle Paare aus 25 Indikatoren auf BTC+ETH+TRX 1d testen
2. Optimization-Set: rangieren nach Performance auf ALLEN 3 Coins gleichzeitig
   (ein Paar ist nur 'gut', wenn es ALLE Coins schlaegt — verhindert Cherry-Pick)
3. Top-10 Paare → Voting-Ensemble auf Holdout testen
4. Mit 3x Hebel + Daily 0.5 % Loss-Limit
"""
import itertools
import warnings

import numpy as np
import pandas as pd

from src.data.storage import load_ohlcv
from src.indicators.technical import (
    sma, ema, rsi, macd, bollinger_pct,
    atr, stochastic, roc, cci, williams_r, obv, adx,
    keltner_pct, aroon, chaikin_money_flow, trend_strength,
)
from src.backtest.engine import triple_barrier_backtest_ls

warnings.filterwarnings("ignore", category=UserWarning)

EXCHANGE = "binance"
INPUT_TIMEFRAME = "1h"
RESAMPLE_TO = "1D"
COINS = ["BTC/USDT", "ETH/USDT", "TRX/USDT"]

STARTING_CAPITAL_CHF = 1000.0
FEE_RATE = 0.001
TP_PCT = 0.05         # 5 % TP — Trend-skaliert
SL_PCT = 0.025        # 2.5 % SL — fuer 2:1 Reward
MAX_HOLD_BARS = 30    # max 30 Tage halten
LEVERAGE = 3.0
DAILY_LOSS_LIMIT = -0.005
COOLDOWN = 1

OPT_FRAC = 0.60
TOP_N_PAIRS = 10
MIN_VOTES_PCT = 0.30   # 30 % der Top-Paare = ~3 von 10
MIN_TRADES_PER_COIN = 5


# Bull/Bear Regeln fuer ~25 Indikatoren
BULL_RULES = {
    "rsi_14":         lambda x: x < 35,
    "bb_pct":         lambda x: x < 0.2,
    "stoch_k":        lambda x: x < 25,
    "williams_r_14":  lambda x: x < -80,
    "cci_20":         lambda x: x < -100,
    "macd_hist":      lambda x: x > 0,
    "ema_ratio":      lambda x: x > 0,
    "sma_ratio":      lambda x: x > 0,
    "roc_10":         lambda x: x > 0,
    "roc_30":         lambda x: x > 0,
    "ret_1":          lambda x: x > 0,
    "ret_3":          lambda x: x > 0,
    "ret_6":          lambda x: x > 0,
    "vol_change":     lambda x: x > 0,
    "obv_slope":      lambda x: x > 0,
    "cmf_20":         lambda x: x > 0,
    "keltner_pct":    lambda x: x < 0.2,
    "aroon_up":       lambda x: x > 70,
    "aroon_osc":      lambda x: x > 50,
    "trend_strength": lambda x: x > 0,
    "trend_strength_long": lambda x: x > 0,
    "adx_14":         lambda x: x > 25,
    "dist_to_20d_high": lambda x: x < 0.02,  # nahe am 20d-Hoch
    "dist_to_20d_low":  lambda x: x > 0.10,  # weit weg vom 20d-Tief
}

BEAR_RULES = {
    "rsi_14":         lambda x: x > 65,
    "bb_pct":         lambda x: x > 0.8,
    "stoch_k":        lambda x: x > 75,
    "williams_r_14":  lambda x: x > -20,
    "cci_20":         lambda x: x > 100,
    "macd_hist":      lambda x: x < 0,
    "ema_ratio":      lambda x: x < 0,
    "sma_ratio":      lambda x: x < 0,
    "roc_10":         lambda x: x < 0,
    "roc_30":         lambda x: x < 0,
    "ret_1":          lambda x: x < 0,
    "ret_3":          lambda x: x < 0,
    "ret_6":          lambda x: x < 0,
    "vol_change":     lambda x: x > 0,
    "obv_slope":      lambda x: x < 0,
    "cmf_20":         lambda x: x < 0,
    "keltner_pct":    lambda x: x > 0.8,
    "aroon_up":       lambda x: x < 30,
    "aroon_osc":      lambda x: x < -50,
    "trend_strength": lambda x: x < 0,
    "trend_strength_long": lambda x: x < 0,
    "adx_14":         lambda x: x > 25,
    "dist_to_20d_high": lambda x: x > 0.10,
    "dist_to_20d_low":  lambda x: x < 0.02,
}


def aggregate_to(df, target):
    return df.resample(target).agg({
        "open": "first", "high": "max", "low": "min",
        "close": "last", "volume": "sum",
    }).dropna()


def compute_indicators(df):
    """Alle Indikatoren in ein DataFrame."""
    out = pd.DataFrame(index=df.index)
    close, high, low, volume = df["close"], df["high"], df["low"], df["volume"]
    out["ret_1"] = close.pct_change(1)
    out["ret_3"] = close.pct_change(3)
    out["ret_6"] = close.pct_change(6)
    out["roc_10"] = roc(close, 10)
    out["roc_30"] = roc(close, 30)
    out["volatility"] = close.pct_change().rolling(24).std()
    out["atr_14"] = atr(high, low, close, 14)
    out["rsi_14"] = rsi(close, 14)
    sk, sd = stochastic(high, low, close, 14, 3)
    out["stoch_k"] = sk
    out["williams_r_14"] = williams_r(high, low, close, 14)
    out["cci_20"] = cci(high, low, close, 20)
    out["macd_hist"] = macd(close)[2]
    out["bb_pct"] = bollinger_pct(close, 20)
    out["sma_ratio"] = sma(close, 10) / sma(close, 50) - 1
    out["ema_ratio"] = ema(close, 12) / ema(close, 26) - 1
    out["adx_14"] = adx(high, low, close, 14)
    out["vol_change"] = volume.pct_change().rolling(6).mean()
    out["obv_slope"] = obv(close, volume).pct_change(10)
    out["cmf_20"] = chaikin_money_flow(high, low, close, volume, 20)
    out["keltner_pct"] = keltner_pct(high, low, close, 20)
    aup, adn, aosc = aroon(high, low, 25)
    out["aroon_up"] = aup
    out["aroon_osc"] = aosc
    out["trend_strength"] = trend_strength(close, 10, 50)
    out["trend_strength_long"] = trend_strength(close, 20, 200)
    out["dist_to_20d_high"] = (high.rolling(20).max() - close) / close
    out["dist_to_20d_low"] = (close - low.rolling(20).min()) / close
    return out


def evaluate_pair_on_coin(close, ind_data, ind_a, ind_b):
    """Backtest fuer 1 Paar auf 1 Coin."""
    bull_a = BULL_RULES[ind_a](ind_data[ind_a]).fillna(False)
    bull_b = BULL_RULES[ind_b](ind_data[ind_b]).fillna(False)
    bear_a = BEAR_RULES[ind_a](ind_data[ind_a]).fillna(False)
    bear_b = BEAR_RULES[ind_b](ind_data[ind_b]).fillna(False)

    p_long = (bull_a & bull_b).astype(float)
    p_short = (bear_a & bear_b).astype(float)

    pos_frac = np.where((p_long > 0.5) | (p_short > 0.5), 0.25, 0.0)
    res = triple_barrier_backtest_ls(
        close=close, p_tp_long=p_long, p_tp_short=p_short,
        entry_threshold=0.5,
        tp_pct=TP_PCT, sl_pct=SL_PCT, max_hold=MAX_HOLD_BARS,
        fee_rate=FEE_RATE, starting_capital=STARTING_CAPITAL_CHF,
        cooldown=COOLDOWN, position_fraction=pos_frac,
        leverage=1.0,
    )
    n_trades = len(res["trades"])
    return {
        "total_return": res["total_return"],
        "n_trades": n_trades,
        "win_rate": (res["trades"]["trade_net_return"] > 0).mean() if n_trades else 0,
    }


def main():
    print(f"Lade {len(COINS)} Coins, aggregiere zu {RESAMPLE_TO} …")
    coins = {}
    for sym in COINS:
        df_1h = load_ohlcv(EXCHANGE, sym, INPUT_TIMEFRAME)
        df = aggregate_to(df_1h, RESAMPLE_TO)
        ind = compute_indicators(df)
        df = pd.concat([df, ind], axis=1).dropna()
        coins[sym] = df
        print(f"  {sym}: {len(df)} Bars von {df.index.min().date()} bis {df.index.max().date()}")

    # Optimization / Holdout split (gleicher Datums-Split fuer alle Coins)
    common_start = max(df.index.min() for df in coins.values())
    common_end = min(df.index.max() for df in coins.values())
    for sym in COINS:
        coins[sym] = coins[sym].loc[common_start:common_end]
    n = len(next(iter(coins.values())))
    opt_end = int(n * OPT_FRAC)
    opt_idx = next(iter(coins.values())).index[opt_end - 1]
    print(f"\nOpt-Set:  {common_start.date()} → {opt_idx.date()}  ({opt_end} Bars)")
    print(f"Holdout:  {opt_idx.date()} → {common_end.date()}  ({n - opt_end} Bars)")

    indicators = list(BULL_RULES.keys())
    pairs = list(itertools.combinations(indicators, 2))
    print(f"\nBrute-Force: {len(pairs)} Paare × {len(COINS)} Coins = {len(pairs) * len(COINS)} Backtests …")

    pair_results = []
    for ind_a, ind_b in pairs:
        per_coin = {}
        valid = True
        for sym in COINS:
            df_opt = coins[sym].iloc[:opt_end]
            res = evaluate_pair_on_coin(df_opt["close"], df_opt, ind_a, ind_b)
            per_coin[sym] = res
            if res["n_trades"] < MIN_TRADES_PER_COIN:
                valid = False
                break
        if not valid:
            continue

        returns = [per_coin[sym]["total_return"] for sym in COINS]
        avg_ret = float(np.mean(returns))
        min_ret = float(min(returns))    # MUSS positiv sein (robust)
        total_trades = sum(per_coin[sym]["n_trades"] for sym in COINS)
        avg_wr = float(np.mean([per_coin[sym]["win_rate"] for sym in COINS]))

        pair_results.append({
            "pair": f"{ind_a}+{ind_b}", "ind_a": ind_a, "ind_b": ind_b,
            "avg_return": avg_ret, "min_return": min_ret,
            "total_trades": total_trades, "avg_wr": avg_wr,
            **{f"ret_{sym.split('/')[0]}": per_coin[sym]["total_return"] for sym in COINS},
        })

    pair_df = pd.DataFrame(pair_results)
    print(f"\n{len(pair_df)} Paare haben mind. {MIN_TRADES_PER_COIN} Trades pro Coin.")

    # ROBUSTER Filter: Paar muss auf ALLEN 3 Coins positiv sein (min_return > 0)
    robust = pair_df[pair_df["min_return"] > 0].sort_values("avg_return", ascending=False)
    print(f"Davon {len(robust)} Paare auf ALLEN 3 Coins profitabel (robust gegen Cherry-Pick).")

    if len(robust) == 0:
        print("\nKEIN einziges Paar war auf allen 3 Coins profitabel.")
        print("Das ist die ehrliche Antwort: Indikator-Paar-Signale haben")
        print("keine reproduzierbare Performance ueber mehrere Coins hinweg.")
        print(f"\nTop-10 nach Durchschnitts-Return (auch wenn nicht ueberall positiv):")
        for _, r in pair_df.sort_values("avg_return", ascending=False).head(10).iterrows():
            print(f"  {r['pair']:<30s} avg {r['avg_return']*100:>+6.2f}%  "
                  f"min {r['min_return']*100:>+6.2f}%  WR {r['avg_wr']*100:.1f}%  "
                  f"Trades {int(r['total_trades'])}")
        return

    print(f"\n=== TOP-{min(TOP_N_PAIRS, len(robust))} ROBUSTE PAARE (auf allen 3 Coins profitabel) ===")
    print(f"  {'Pair':<32s} {'AvgRet':>7s} {'MinRet':>7s} {'WR':>5s} {'Trades':>6s}  "
          f"{'BTC':>7s} {'ETH':>7s} {'TRX':>7s}")
    top_pairs = robust.head(TOP_N_PAIRS)
    for _, r in top_pairs.iterrows():
        print(f"  {r['pair']:<32s} {r['avg_return']*100:>+6.2f}% {r['min_return']*100:>+6.2f}% "
              f"{r['avg_wr']*100:>4.1f}% {int(r['total_trades']):>6d}  "
              f"{r['ret_BTC']*100:>+6.2f}% {r['ret_ETH']*100:>+6.2f}% {r['ret_TRX']*100:>+6.2f}%")

    # --- Holdout Voting Ensemble ---
    print(f"\n--- HOLDOUT Ensemble: {len(top_pairs)} Paare, Threshold {MIN_VOTES_PCT*100:.0f}% Votes ---")
    holdout_results = {}
    for sym in COINS:
        df_h = coins[sym].iloc[opt_end:]
        long_votes = pd.Series(0, index=df_h.index)
        short_votes = pd.Series(0, index=df_h.index)
        for _, r in top_pairs.iterrows():
            a, b = r["ind_a"], r["ind_b"]
            long_votes += (BULL_RULES[a](df_h[a]).fillna(False)
                           & BULL_RULES[b](df_h[b]).fillna(False)).astype(int)
            short_votes += (BEAR_RULES[a](df_h[a]).fillna(False)
                            & BEAR_RULES[b](df_h[b]).fillna(False)).astype(int)
        p_long = long_votes / len(top_pairs)
        p_short = short_votes / len(top_pairs)
        pos_frac = np.where((p_long > MIN_VOTES_PCT) | (p_short > MIN_VOTES_PCT),
                            np.maximum(p_long, p_short).values * 0.30, 0.0)
        res = triple_barrier_backtest_ls(
            close=df_h["close"], p_tp_long=p_long, p_tp_short=p_short,
            entry_threshold=MIN_VOTES_PCT,
            tp_pct=TP_PCT, sl_pct=SL_PCT, max_hold=MAX_HOLD_BARS,
            fee_rate=FEE_RATE, starting_capital=STARTING_CAPITAL_CHF / len(COINS),
            cooldown=COOLDOWN, position_fraction=pos_frac,
            leverage=LEVERAGE, daily_loss_limit=DAILY_LOSS_LIMIT,
        )
        holdout_results[sym] = res

    print(f"\n  {'Coin':>10s}  {'Endkap.':>10s}  {'Return':>9s}  {'Trades':>7s}  {'WR':>6s}  {'MaxDD':>8s}")
    total_final = 0
    total_bh = 0
    for sym in COINS:
        res = holdout_results[sym]
        trades = res["trades"]
        eq = res["equity"]
        wr = (trades["trade_net_return"] > 0).mean() if len(trades) else 0
        max_dd = (eq / eq.cummax() - 1).min() if len(eq) else 0
        df_h = coins[sym].iloc[opt_end:]
        bh_ret = (df_h["close"].iloc[-1] / df_h["close"].iloc[0] - 1)
        cap0 = STARTING_CAPITAL_CHF / len(COINS)
        print(f"  {sym:>10s}  {res['final_capital']:>10.2f}  {res['total_return']*100:>+8.2f}%  "
              f"{len(trades):>7d}  {wr*100:>5.1f}%  {max_dd*100:>+7.2f}%")
        total_final += res["final_capital"]
        total_bh += cap0 * (1 + bh_ret)

    print(f"\n  {'GESAMT':>10s}  {total_final:>10.2f}  {(total_final/STARTING_CAPITAL_CHF-1)*100:>+8.2f}%  "
          f"(B&H Portfolio: {total_bh:.2f} = {(total_bh/STARTING_CAPITAL_CHF-1)*100:+.2f}%)")


if __name__ == "__main__":
    main()
